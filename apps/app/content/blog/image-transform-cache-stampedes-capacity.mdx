...
`proxy_cache_lock_timeout` must be shorter than the user-facing deadline. `proxy_cache_lock_age` is an escape hatch: after it expires, Nginx may allow another origin fill. Set it above the normal high-percentile cold-transform duration, then alert when it is exercised. An unlimited lock wait merely moves the unbounded queue into the proxy.
