...
Also test an absent object, slow origin, redirect, oversized input, and unsupported format. Keenpix currently documents `502` for origin failure, `504` for origin timeout, `413` for an over-size origin, and `503` when transform concurrency is saturated. Your UI needs an explicit fallback or error state; a provider change does not create one automatically.
