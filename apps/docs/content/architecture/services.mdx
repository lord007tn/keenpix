...
| `apps/delivery-edge` | Cloudflare Workers | First-party and customer hostname routing, cache partitioning, and edge analytics | Cloudflare handles scaling |
...
Hosted delivery and the delivery edge target `apps/transform` directly. In self-host mode, `apps/app` can proxy transform requests to preserve the traditional single public hostname while the data plane remains independently scalable.
