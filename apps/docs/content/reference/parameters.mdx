...
| `sig` | string | base64url HMAC-SHA256 | required when the project enables [signed URLs](/docs/concepts/signed-urls) | Request signature over the source URL + sorted params |
...
- **Client Hints.** `w=auto&dpr=auto` lets the browser provide `Width` or
  `Sec-CH-Viewport-Width` plus `Sec-CH-DPR`. `Sec-CH-Width` is already in device
  pixels, so Keenpix converts it back to a logical width before applying DPR and
  avoids double scaling. Keenpix advertises and varies by all three current
  headers; the older unprefixed forms are accepted for compatibility. Automatic
  widths and DPRs are rounded to a bounded shared ladder so a signed `auto` URL
  cannot be turned into unlimited cache variants by spoofing headers. Explicit
  numeric values remain best for deterministic prewarming.
...
- **Watermarks are project configuration, not public query parameters.** Configure
  the asset, opacity, scale, position, and margin under **Settings → Pipeline**.
  The asset host must be allowlisted. Raster variants receive the overlay; explicit
  SVG output remains optimized SVG without raster compositing.
