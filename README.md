...
Keenpix releases use one semantic version tag (`vMAJOR.MINOR.PATCH`, e.g. `v0.3.1`) for the platform, Docker images, SDK, and every public framework package. Before tagging, align the five private app/edge package versions plus the public package versions and author `.github/release-notes/vMAJOR.MINOR.PATCH.md` using the [release-note template](./.github/release-notes/README.md). Pushing the tag validates the structured, self-contained notes before publishing public packages, then creates one GitHub Release grouped by component through [release.yml](./.github/workflows/release.yml). Package-specific tags and GitHub Releases are not used. Docker builds and publishing remain intentionally manual-only: run [docker.yml](./.github/workflows/docker.yml) on the reviewed tag and explicitly enable its publish input to push all four GHCR images as `vX.Y.Z` and `vX.Y`.
