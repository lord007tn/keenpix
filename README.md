...
Keenpix releases use one semantic version tag (`vMAJOR.MINOR.PATCH`, e.g. `v0.3.1`) for the platform, Docker images, SDK, and every public framework package. Before tagging, align the five private app/edge package versions plus the public package versions and author `.github/release-notes/vMAJOR.MINOR.PATCH.md` using the [release-note template](./.github/release-notes/README.md). Pushing the tag validates the structured, self-contained notes before publishing public packages, then creates one GitHub Release grouped by component through [release.yml](./.github/workflows/release.yml). npm publishing authenticates with the GitHub Actions OIDC identity for `lord007tn/keenpix` and `release.yml`; it does not use a long-lived npm token. Package-specific tags and GitHub Releases are not used. Docker builds and publishing remain intentionally manual-only: run [docker.yml](./.github/workflows/docker.yml) on the reviewed tag and explicitly enable its publish input to push all four GHCR images as `vX.Y.Z` and `vX.Y`.

Public `@keenpix/*` integration packages use Changesets to record package-facing work, while the complete package family and platform are released on the same version. Run `pnpm changeset` for a package-facing change and describe the final user impact directly in the versioned release-note file. The package family is versioned together so adapters cannot drift away from the shared core contract.
