...
export interface OriginFetchLimits {
  maxBytes: number
  timeoutMs: number
}
...
function requestOrigin(origin: SafeOrigin, timeoutMs: number) {
...
export async function fetchOriginImage(
  ...
  limits: OriginFetchLimits,
) {
  let current = start
  for (let hop = 0; hop < 4; hop++) {
    try {
      const stream = requestOrigin(current, limits.timeoutMs)
      ...
      return await readCapped(stream, limits.maxBytes)
    } catch (error) {
